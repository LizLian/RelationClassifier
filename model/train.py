...
def train_classifier(vocabulary: 'Vocabulary', transformer: 'BasicTransform', data: List[int], data_test=None,
                     ctx=mx.cpu()) -> List[int]:
    """
    ...
    # set k to a smaller number when you want to test the model on a smaller dataset
    k = 1000
...
class DistanceLoss(nn.Block):
    ...
    def forward(self, score: List[float], label: List[int], mplus=2.5, mNeg=0.5, gamma=2) -> List[float]:
        """
        ...
        part2 = mx.nd.dot(predT.astype('float'), part2[:, 1].astype('float')) + \
                mx.nd.dot(predF.astype('float'), part2[:, 0].astype('float')) # positive loss
...
def _eval(model: 'RelationClassifier', dataloader: 'DataLoader', ctx=mx.cpu()) -> int:
    """
...
if __name__ == '__main__':
    ...
    if args.embedding_source:
        ## experiment different embeddings here (word2vec, fasttext, glove, skipgram etc.)
    ...
    preds = train_classifier(vocab, transform, dataset, test_dataset, ctx)
